...
public class ExecServiceTest {
	...
	private static final Dictionary<String, Object> getProperties(long delay, long period) {
		...
		dict.put(ExecutorConstants.TASK_DELAY, period);
		dict.put(ExecutorConstants.TASK_PERIOD, period);
		return dict;
	}
	...
	public Option[] bundles() throws IOException {
		return new Option[] {
				// leads to a strange ClassCastException in Pax Exam, see 
//				CoreOptions.frameworkProperty("org.osgi.framework.system.packages.extra").value("sun.reflect,sun.misc,javax.annotation"),
...
//				CoreOptions.mavenBundle("org.apache.felix", "org.apache.felix.useradmin.filestore", "1.0.2"),
	...
	protected Map<String, Object> configProperty() {
		final Map<String, Map<String, Object>> properties0 = Arrays
				.stream(new String[] { ExecutorConstants.HOUSEKEEPING_EXEC_PID })
	...
	public void unitWorks() throws InterruptedException {
		...
		dict.put(ExecutorConstants.TASK_PROPERTIES_TIME_UNIT, ChronoUnit.SECONDS.name());

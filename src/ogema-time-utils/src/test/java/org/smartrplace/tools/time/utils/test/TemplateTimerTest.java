...
import org.smartrplace.tools.time.utils.templates.TemplateTimer;
import org.smartrplace.tools.time.utils.templates.TemplateTimer.TemplateTimerListener;
import org.smartrplace.tools.time.utils.templates.simple.SimpleTemplateManagement;
import org.smartrplace.tools.time.utils.templates.simple.SimpleTemplateTimer;
...
public class TemplateTimerTest extends LatestVersionsTestBase {
	...
	private void templateWorksSingleDay(final Map<LocalTime, Integer> map, DayOfWeek day) { 
		final SimpleTemplateManagement<Integer> templateMgmt = new SimpleTemplateManagement<>();
		if (day == null)
		...
		final TestListener listener = new TestListener(map.size());
	...
	@Test
	public void templatingWorksDefaultSingleDay() {
		...
		templateWorksSingleDay(map, null);
	}
	
	@Test
	public void templatingWorksSpecialDay() {
		final AtomicInteger cnt = new AtomicInteger(0);
		final Map<LocalTime, Integer> map = new TreeMap<>(Stream.<LocalTime> builder()
				.add(LocalTime.of(3, 17))
				.add(LocalTime.of(7, 23))
				...
				.build()
				.collect(Collectors.toMap(Function.identity(), (d) -> cnt.getAndIncrement())));
		templateWorksSingleDay(map, DayOfWeek.THURSDAY);
	}
	
	@Test
	public void templatingWorksMultipleDays() { 
		final SimpleTemplateManagement<Integer> templateMgmt = new SimpleTemplateManagement<>();
		final AtomicInteger cnt = new AtomicInteger(0);
		...
		getApplicationManager().getAdministrationManager().getFrameworkClock().setSimulationTime(-1);
		final TestListener listener = new TestListener(sz);
		final SimpleTemplateTimer<Integer> timer = new SimpleTemplateTimer<Integer>(getApplicationManager(), templateMgmt.getPovider(), null, zone);
		try {
			timer.addTemplateTimerListener(listener); // FIXME add constrcutor to add template listener immediately
			final float factor = 24 * 60 * 60 / 5; // factor to let one day pass in 5 seconds
			final boolean done = executeAtSimulationSpeed(factor, () -> listener.await(30, TimeUnit.SECONDS)); // expected: 15s
	...
	private static class TestListener implements TemplateTimerListener<Integer> {
		...
		TestListener(int expectedCallbacks) {
